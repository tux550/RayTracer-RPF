...
std::vector<std::vector<BasicRGB>> createBasicRGBMatrix(size_t nRows,
                                                        size_t nCols) {
    return std::vector<std::vector<BasicRGB>>(
        nRows, std::vector<BasicRGB>(nCols, {0, 0, 0}));
}

void writeRGBMatrix(const std::vector<std::vector<BasicRGB>> &rgbMatrix,
    ...
    for (size_t i = 0; i < nRows; ++i) {
        for (size_t j = 0; j < nCols; ++j) {
            pixels[j * nRows + i] = Imf::Rgba(
                rgbMatrix[i][j].r, rgbMatrix[i][j].g, rgbMatrix[i][j].b, 1);
        }
    }
...
void normalizeRGBMatrix(std::vector<std::vector<BasicRGB>> &rgbMatrix) {
    // Find max values
