...
struct SampleData {
    ...
    // === GETTERS ===
    Point2f getPFilm() const { return Point2f(data[0], data[1]); }
    Point2f getPLens() const { return Point2f(data[5], data[6]); }
    Spectrum getL() const {
        // Fix: float casting
        Float rgb[3] = {data[2], data[3], data[4]};
        return Spectrum::FromRGB(rgb);
    }
    Normal3f getN0() const { return Normal3f(data[7], data[8], data[9]); }
    Point3f getP0() const { return Point3f(data[10], data[11], data[12]); }
    Normal3f getN1() const { return Normal3f(data[13], data[14], data[15]); }
    Point3f getP1() const { return Point3f(data[16], data[17], data[18]); }

    // CONSTRUCTORS
    SampleData() : data(), rayWeight(0) {
        for (int i = 0; i < 19; i++) {
            data[i] = 0;
        }
    };

    SampleData(const Point2f &pFilm, const Point2f &pLens, const Spectrum &L,
               Float rayWeight)
        : data(), rayWeight(rayWeight) {
        setPFilm(pFilm);
        setL(L);
        setPLens(pLens);
    };
    ...
    SampleF getFeatures() const { return getFeatures(data); }
    double getFeatureI(int i) const { return data[i + 7]; }
    ...
    SampleP getPosition() const { return getPosition(data); }
    double getPositionI(int i) const { return data[i]; }
    ...
    SampleR getRandom() const { return getRandom(data); }
    double getRandomI(int i) const { return data[i + 5]; }
    ...
    SampleC getColor() const { return getColor(data); }
    double getColorI(int i) const { return data[i + 2]; }
    void setColorI(int i, double value) { data[i + 2] = value; }
    ...
    // === FULL ARRAY ===
    SampleFullArray getFullArray() const { return data; }
    void setFullArray(const SampleFullArray &fullArray) { data = fullArray; }

    // === UTILITY ===
    // Normalize using mean and stdDev
    SampleData normalized(const SampleFullArray &mean,
                          const SampleFullArray &stdDev) const {
        SampleData normalizedData;
        normalizedData.data = divideArrays(subtractArrays(data, mean), stdDev);
