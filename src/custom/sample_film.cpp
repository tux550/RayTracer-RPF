...
namespace pbrt {
  ...
  SamplingTile::SamplingTile(const Bounds2i &pixelBounds) : pixelBounds(pixelBounds) {
    samples = std::vector<std::vector<SampleDataSet>>(
      pixelBounds.Diagonal().x,
      std::vector<SampleDataSet>(
  ...
  std::unique_ptr<SamplingTile> SamplingFilm::GetSamplingTile(const Bounds2i &sampleBounds) {
